...
telemetry calls. The repository remains a pre-release prototype and has not been
initialized or published as a Git repository by this implementation task.

Machine-verifiable release checks are kept inside this clean-room directory. The
Python 3.12 synthetic suite exercises transactional state invariants, restart and
delivery reconciliation, path and file-mode defenses, deterministic CLI status,
and canary absence from stdout, stderr, SQLite, WAL, and SHM. The values-free
...
detected without being reproduced in audit output.
